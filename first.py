...
class Application:
    ...
    def search_db(self):
        ...
        conn = mysql.connector.connect(host=hostName,
                     ...
                     database=db_database)
        c = conn.cursor()
        sql = "SELECT * FROM appointment WHERE name LIKE %s"
        print(self.input)
...
##        self.delete.place(x=150, y=390)
    def update_db(self):
        ...
        query = "UPDATE appointment SET name=%s, age=%s, gender=%s, address=%s, phone=%s, time=%s WHERE name LIKE %s"
    ...
    def delete_db(self):
        conn = mysql.connector.connect(host=hostName,
                     ...
                     database=db_database)
        c = conn.cursor()
        sql2 = "DELETE FROM appointment WHERE name LIKE %s"
        c.execute(sql2, [(self.input)])
        conn.commit()
